...
// Object. The transport is ported from charwork/functions/mcp.js, which is already right on the
...
// and MCP surfaces cannot describe different behaviour. They are also distinct from the
// in-page tools in webmcp.js, which play the game rather than read the quote set.
  ...
  switch (method) {
    case 'initialize':
      return reply({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'quotable', version: VERSION },
      });
    ...
    case 'tools/call': {
      ...
      try {
        ...
        if (err instanceof ToolError) {
          return reply({ content: [{ type: 'text', text: `quotable: ${err.message}` }], isError: true });
        }
        throw err;
      }
      if (!result) {
